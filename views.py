...
from .models import UserPolygon, ProjectConfiguration
...
class AvailableDatesView(APIView):
    ...
    def get(self, request):
        ...
        try:
            # --------------------------------------------------
            # 1?? Read configuration FIRST (source of truth)
            # --------------------------------------------------
            ...
            # --------------------------------------------------
            # 2?? Build cache key (ONLY after config is known)
            # --------------------------------------------------
            cache_key = (
                project.strip().lower(),
            ...
            # --------------------------------------------------
            # 3?? Optional cache lookup (TTL > 0 only)
            # --------------------------------------------------
            ...
            # --------------------------------------------------
            # 6?? Fetch DISTINCT dates (newest first)
            # --------------------------------------------------
            with engine.connect() as conn:
                rows = conn.execute(
                    text(
...
def resolve_table_any(engine, raw_name: str, db_label: str):
    ...
    with engine.connect() as conn:
        # --------------------------------------------------
        # 1?? Prefer BASE TABLES (public first)
        # --------------------------------------------------
        row = conn.execute(
            text(
                """
        ...
        # --------------------------------------------------
        # 2?? Fallback to VIEWS (public first)
        # --------------------------------------------------
        if not row:
            row = conn.execute(
                text(
...
class DistinctValuesView(APIView):
    ...
    def get(self, request, table):
        ...
        try:
            ...
            # --------------------------------------------------
            # 1?? Read project configuration (if exists)
            # --------------------------------------------------
            source_table = None
            target_table = None
            date_col = None
            ...
            # --------------------------------------------------
            # 3?? Decide table by COLUMN OWNERSHIP
            # --------------------------------------------------
            resolved_table = None
            resolved_db = None
            ...
            # --------------------------------------------------
            # 4?? FALLBACK: direct table usage (NO PROJECT CONFIG)
            # --------------------------------------------------
            ...
            # --------------------------------------------------
            # 5?? Normal project-based execution
            # --------------------------------------------------
            engine = get_engine_for_db(resolved_db)
...
class ProjectConfigurationListView(APIView):
    print("new class")
    permission_classes = [AllowAny]

    def get(self, request):
        try:
            configs = ProjectConfiguration.objects.all()
            data = []
            for config in configs:
                data.append({
                    ...
                    'thersold1': config.thersold1,
                    # 'column2': config.column2,
                    # 'thersold2': config.thersold3,
                    'column3': config.column3,
                    'thersold3': config.thersold3,
                    'column4': config.column4,
                    'thersold4': config.thersold4,
                    'column5': config.column5,
                    'thersold5': config.thersold5,
                    'column6': config.column6,
                    'thersold6': config.thersold6,
                    'column7': config.column7,
                    'thersold7': config.thersold7,
                    'column8': config.column8,
                    'thersold8': config.thersold8,
                    'column9': config.column9,
                    'thersold9': config.thersold9,
                    'column10': config.column10,
                    'thersold10': config.thersold10,
                    'column11': config.column11,
                    'thersold11': config.thersold11,
                })
            return Response({'configurations': data}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({'detail': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)


class KPIFailurePerTechView(APIView):
    permission_classes = [AllowAny]
    ...
    def get(self, request):
        ...
        logger.warning("========== KPI FAILURE API CALLED ==========")
        logger.warning(f"[INPUT] project={project}")
        logger.warning(f"[INPUT] query_params={dict(request.query_params)}")
        ...
        #  CONFIG (HARDCODED)
        # ===============================
        KPI_TABLE = "BHAZ01_RPRT_ALL_KPI_FAIL_CELL_COUNT"
        DATE_COL = "Date"
        TECH_COL = "Tech"

        try:
            set_progress("Loading KPI Failure per Tech")
            start_dot_animation()

            # =====================================================
            #  Step 1: Resolve KPI table
            # =====================================================
            kpi_db = find_db_for_table_cached(KPI_TABLE)
            logger.warning(f"[KPI DB] resolved_db={kpi_db}")

            if not kpi_db:
                raise Exception(f"{KPI_TABLE} not found in any DB")

            kpi_engine = get_engine_for_db(kpi_db)

            qualified_kpi, k_schema, k_table = resolve_table_any(
                kpi_engine, KPI_TABLE, kpi_db
            )

            logger.warning(f"[KPI TABLE] qualified={qualified_kpi}")

            # =====================================================
            #  Step 2: Date logic (LATEST DATE DEFAULT)
            # =====================================================
            from_date, to_date = read_date_filters_from_request(request)
            selected_dates = read_multi_dates_from_request(request)
            ...
            if selected_dates:
                where.append(f'"{DATE_COL}"::date IN :dates')
                params["dates"] = tuple(selected_dates)

            elif from_date and to_date:
                where.append(f'"{DATE_COL}" BETWEEN :from_date AND :to_date')
                params["from_date"] = from_date
                params["to_date"] = to_date
                ...
                #  DEFAULT → latest date
                latest_sql = text(
                    f'SELECT MAX("{DATE_COL}") AS max_date FROM {qualified_kpi}'
                )
                latest_date = pd.read_sql(
                    latest_sql, kpi_engine.connect()
                )["max_date"].iloc[0]

                logger.warning(f"[DATE] latest_date_used={latest_date}")

                if latest_date is not None:
                    where.append(f'"{DATE_COL}" = :latest_date')
            ...
            kpi_df = pd.read_sql(kpi_sql, kpi_engine.connect(), params=params)

            logger.warning(f"[KPI DF] rows={len(kpi_df)}")

            # ===============================
            #  FORCE LATEST DATE (if no filter)
            # ===============================
            if not selected_dates and not from_date and not to_date:
                if DATE_COL in kpi_df.columns:
                    latest_date = pd.to_datetime(kpi_df[DATE_COL]).max()
                    logger.warning(f"[LATEST DATE AUTO] using {latest_date}")

                    kpi_df = kpi_df[
                        pd.to_datetime(kpi_df[DATE_COL]) == latest_date
                    ]

            # ===============================
            #  TECH FILTER
            # ===============================
            selected_techs = request.query_params.getlist("tech")
            logger.warning(f"[TECH FILTER] {selected_techs}")

            if selected_techs and TECH_COL in kpi_df.columns:
                kpi_df = kpi_df[
                    kpi_df[TECH_COL].astype(str).isin(selected_techs)
                ]
            ...
            # =====================================================
            #  Step 4: Detect important KPI columns
            # =====================================================
            ...
            kpi_cell_col = find_col(kpi_df, ["cell"])
            kpi_site_col = find_col(kpi_df, ["site"])
            kpi_fail_col = find_col(kpi_df, ["fail"])

            logger.warning(
                f"[KPI COLS] cell={kpi_cell_col}, site={kpi_site_col}, fail={kpi_fail_col}"
            )

            if not kpi_cell_col or not kpi_fail_col:
                raise Exception("Required KPI columns not found")

            # =====================================================
            #  Step 5: Resolve SOURCE TABLE (DB-level aware)
            # =====================================================
            source_db = infer_db_from_project(project)
            logger.warning(f"[SOURCE DB] inferred_db={source_db}")

            base_project = project.split("_")[0].strip().lower()

            with config_engine.connect() as conn:
                row = conn.execute(
                    text(
                        """
                        SELECT source_table, project_name
                        FROM geolytics_projectconfiguration
                        WHERE lower(project_name) LIKE :p
                          AND source_table IS NOT NULL
                        LIMIT 1
                        """
                    ),
            ...
            if not row:
                raise Exception(f"No source_table mapping found for DB {project}")

            source_table = row[0].strip()
            logger.warning(
                f"[SOURCE RESOLVED] using project={row[1]} table={source_table}"
            )
            ...
            def find_src(cands):
                for name in src_cols:
                    ln = name.lower()
                    if any(c in ln for c in cands):
                        return name
                return None

            lat_col = find_src(["lat", "latitude"])
            lon_col = find_src(["lon", "long", "longitude"])
            site_col = find_src(["site"])
            cell_col = find_src(["cell"])

            logger.warning(
                f"[SOURCE DETECT] lat={lat_col}, lon={lon_col}, site={site_col}, cell={cell_col}"
            )

            if not lat_col or not lon_col:
                raise Exception("Lat/Lon not found in source")
            ...
            src_sql = text(
                f'''
                SELECT
                    "{cell_col}" AS cellname,
                    "{site_col}" AS site_name,
                    "{lat_col}" AS "Lat",
                    "{lon_col}" AS "Long"
                FROM {qualified_source}
            ...
            src_df = pd.read_sql(src_sql, source_engine.connect())

            logger.warning(f"[SOURCE DF] rows={len(src_df)}")

            # =====================================================
            #  Step 8: Merge
            # =====================================================
            kpi_df[kpi_cell_col] = kpi_df[kpi_cell_col].astype(str)
            src_df["cellname"] = src_df["cellname"].astype(str)
            ...
            merged = pd.merge(
                ...
                kpi_df,
                left_on="cellname",
                right_on=kpi_cell_col,
                how="left",
            )

            logger.warning(
                f"[MERGE] matched_failures={merged[kpi_fail_col].notna().sum()}"
            )

            # =====================================================
            #  Step 9: Issue bucket coloring (dynamic)
            # =====================================================
            ISSUE_COL = "Issue/Analysis Bucket new"
            ...
            logger.warning(f"[ISSUE BUCKETS] {unique_issues}")
            ...
            issue_color_map = {
                issue: palette[i % len(palette)]
                for i, issue in enumerate(sorted(unique_issues))
            }

            merged["color"] = merged[ISSUE_COL].map(issue_color_map).fillna("#9ca3af")

            # =====================================================
            #  Step 10: Build GeoJSON
            # =====================================================
            features = []
            ...
            for _, r in merged.iterrows():
                try:
                    ...
                    #  IMPORTANT  tech used by legend filter
                    if TECH_COL in r:
                        props["tech"] = str(r.get(TECH_COL)).upper()
            ...
            logger.warning(f"[FINAL] features_generated={len(features)}")
            logger.warning("========== KPI FAILURE API SUCCESS ==========")

            return Response({
                "type": "FeatureCollection",
                "features": features,
                "color_mode": "kpi_failure_per_techch",
                "issue_legend": issue_color_map,
            })

        except Exception as e:
            stop_dot_animation()
            reset_progress_later()
            logger.exception("KPIFailurePerTechView failed")
            return Response({"error": str(e)}, status=500)
